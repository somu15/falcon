...
registerMooseObject("FalconApp", TestMC);
...
TestMC::TestMC(const InputParameters & parameters)
  : Sampler(parameters),
...
std::vector<Real>
TestMC::computeCoords(const Real & p, const Real & p2, const Real & Radius, const std::vector<Real> & center_coords, const std::vector<Real> & basis1, const std::vector<Real> & basis2, const std::vector<Real> & invM, const Real & plane_const)
{
...
Real
TestMC::computeSample(dof_id_type /*row_index*/, dof_id_type col_index)
{
